...
from tools.DataReader import Datasets
...
class Constructor:
    def __init__(self, model, model_name='MyNet'):

        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.model = model.to(device=self.device)
        self.model_name = model_name
        self.optimizer = optim.AdamW(self.model.parameters(), lr=0.0018, weight_decay=0)
        self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer=self.optimizer, patience=8)
    ...
    def practise(self, TrainLoader, ValidateLoader):
        ...
        torch.save(self.model.state_dict(), path + '/save_models1/' + self.model_name + '_' + self.data_name + '.pth')


    def demonstration(self, TestLoader):
        path = os.path.abspath(os.curdir)
        self.model.load_state_dict(torch.load(path + '/save_models1/' + self.model_name + '_' + self.data_name + '.pth', map_location=self.device, weights_only=True))
        predict_value = []
        true_label = []
        self.model.eval()
        with torch.no_grad():
    ...
    def estimate(self, predict_value, true_label):
        ...
        precision, recall, _ = precision_recall_curve(y_score=predict_value, y_true=true_label)
...
roc_auc = []
pr_auc = []
data_path = './tools/FILESLIST1.txt'
i = 1
...
with open(data_path, 'r') as file:
    for line in file:
        ...
        i = i + 1
        name = line.strip()
        Train = Constructor(model=MyNet(), model_name='MyNet')
...
with open('results.csv', 'w', newline='') as csvfile:
    ...
    for i in range(len(acc)):
        writer.writerow([acc[i], roc_auc[i], pr_auc[i]])

    writer.writerow(['Average', sum(acc)/len(acc), sum(roc_auc)/len(roc_auc), sum(pr_auc)/len(pr_auc)])